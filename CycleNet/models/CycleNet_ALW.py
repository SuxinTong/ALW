...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.model_type == 'linear':
            self.model = nn.Linear(self.red_len, self.pred_len)
        elif self.model_type == 'mlp':
            self.model = nn.Sequential(
                nn.Linear(self.red_len, self.d_model),
                nn.ReLU(),
                nn.Linear(self.d_model, self.pred_len)
            )

    def forward(self, x, cycle_index):
        # x: (batch_size, red_len, enc_in), cycle_index: (batch_size,)
        ...
        ALW, means, stdev = self.ALW(x)
        
        x = ALW.permute(0, 2, 1)

        # remove the cycle of the input data
        x = x - self.cycleQueue(cycle_index, self.red_len)
        ...
        y = y + self.cycleQueue((cycle_index + self.red_len) % self.cycle_len, self.pred_len)
