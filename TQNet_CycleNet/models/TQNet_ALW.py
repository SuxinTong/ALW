...
from layers.ALW import ALW

class Model(nn.Module):
    def __init__(self, configs):
        super(Model, self).__init__()

        self.red_len = configs.red_len
        ...
        if self.channel_aggre:
            self.channelAggregator = nn.MultiheadAttention(embed_dim=self.red_len, num_heads=4, batch_first=True, dropout=0.5)

        self.input_proj = nn.Linear(self.red_len, self.d_model)
    ...
    def forward(self, x, cycle_index):
        ...
        x_input, means, stdev = self.ALW(x)

        if self.use_tq:
            gather_index = (cycle_index.view(-1, 1) + torch.arange(self.red_len, device=cycle_index.device).view(1, -1)) % self.cycle_len
        ...
        if self.use_revin:
            output = output * torch.sqrt(stdev) + means
        elif self.use_norm:
